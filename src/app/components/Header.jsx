...
import logo from "../../../public/images/logo-your-reivew.png";
  ...
  return (
    ...
    <header className="bg-[#edf0f5] hidden md:block">
      <div className="mx-auto flex items-center justify-between px-4 py-3 md:py-5 max-w-[1050px]">
        ...
        <Link href="/" className="flex items-center space-x-2">
          <Image src={logo} alt="" width={200} className="object-cover" />
        </Link>

        {/* Center Navigation */}
        <nav className="hidden md:flex items-center space-x-8">
          <Link href="/pricing" className="text-gray-800 hover:text-[#713fff]">
            Plans
          </Link>
          <Link href="/reviews" className="text-gray-800 hover:text-[#713fff]">
            Reviews
          </Link>
          <Link
            href="/resources"
    ...
    <header className=" block md:hidden">
      <div className="mx-auto my-4 flex items-center justify-between px-3 py-3 md:py-3 max-w-[1050px] shadow-2xl bg-white rounded-full w-90">
        ...
        <AlignJustify className="text-[#878787] h-8 w-8"  onClick={() => setIsOpen(!isOpen)} />
      </div>
    </header>
